  ...
  context "constraints which is for attributes of the model" do
    context "generation" do
      it "raise no error" do
        expect(instance.class.connection.execute("SELECT * FROM pg_constraint WHERE conname='valogy_slots_sheet_id_minmal' ").ntuples).to eq(0)
        expect{Valogy::Parser.parse("#{fixture_path}/test.owl")}.not_to raise_error
        expect(instance.class.connection.execute("SELECT * FROM pg_constraint WHERE conname='valogy_slots_sheet_id_minimal' ").ntuples).to eq(1)
      end
    end
